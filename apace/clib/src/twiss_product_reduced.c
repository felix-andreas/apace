...
void twiss_product(int n, int size, double (*A)[size][size], double(*B0), double (*twiss_array)[n]) {
  int pos;
  double(*m)[5];
  for (pos = 0; pos < n; pos++) {
    m = A[pos];
...
//    for ( pos = 0; pos < n; pos ++ ) {
//        m = A[pos];
//        tmp[0] = m[0][0] * m[0][0] * B0[0] - 2. * m[0][0] * m[0][1] * B0[2] +
//        m[0][1] * m[0][1] * B0[4]; tmp[1] = m[2][2] * m[2][2] * B0[1] - 2. *
//        m[2][2] * m[2][3] * B0[3] + m[2][3] * m[2][3] * B0[5]; tmp[2]
//        =-m[0][0] * m[1][0] * B0[0] + (m[0][0] * m[1][1] + m[0][1] * m[1][0])
//        * B0[2] - m[1][1] * m[0][1] * B0[4]; tmp[3] =-m[2][2] * m[3][2] *
//        B0[1] + (m[2][2] * m[3][3] + m[2][3] * m[3][2]) * B0[3] - m[3][3] *
//        m[2][3] * B0[5]; tmp[4] = m[1][0] * m[1][0] * B0[0] - 2. * m[1][1] *
//        m[1][0] * B0[2] + m[1][1] * m[1][1] * B0[4]; tmp[5] = m[3][2] *
//        m[3][2] * B0[1] - 2. * m[3][3] * m[3][2] * B0[3] + m[3][3] * m[3][3] *
//        B0[5]; tmp[6] = m[0][0] * B0[6]  + m[0][1]* B0[7] + m[0][4]; tmp[7] =
//        m[1][0] * B0[6]  + m[1][1]* B0[7] + m[1][4];
//    }
