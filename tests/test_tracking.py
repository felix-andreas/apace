...
def test_tune(fodo_ring):
    ...
    tracking = ap.MatrixTracking(fodo_ring, dist, turns=n_turns, watch_points=[0])
...
def test_particle_trajectory(fodo_ring):
    ...
    tracking = ap.MatrixTracking(fodo_ring, dist, turns=n_turns, watch_points=None)
    ...
    print(x[idx_test, 0])
    print(emittance_sqrt * np.sqrt(beta_x[idx_test]) * np.cos(psi_x[idx_test]))
    assert np.allclose(
        x[idx_test, 0],
