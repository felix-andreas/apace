...
colors = {"Bend": "#ffc425", "Quad": '#d11141', "Sext": "#ffc425'"}
fs = 9
cmap = mpl.cm.Set1
...
def plot_twiss(ax, twiss, linestyle="solid"):
    ax.plot(twiss.s, twiss.betax, color=cmap(0 / 9), linewidth=1.3, linestyle=linestyle)
    ax.plot(twiss.s, twiss.betay, color=cmap(1 / 9), linewidth=1.3, linestyle=linestyle)
    ax.plot(twiss.s, twiss.etax, color=cmap(2 / 9), linewidth=1.3, linestyle=linestyle)
...
def set_limits(ax, maincell, xmin=None, xmax=None, ymin=None, ymax=None):
    ...
    ymax = ymax if ymax else 1.2 * ylim[1]
...
def set_grid(ax, maincell):
    ...
    ax.set_xticks(np.add.accumulate(cell_length_list), minor=True)
    ax.set_xticks(np.linspace(0, maincell.length, len(cell_length_list), endpoint=True))
...
def plot_full(ax,
              ...
              linestyle="solid",
              ref_twiss=None,
              ref_linestyle="dashed"):


    maincell = linbeamdyn.maincell

    if ref_twiss:
        plot_twiss(ax, ref_twiss.twiss, linestyle=ref_linestyle)
    plot_twiss(ax, linbeamdyn.twiss, linestyle=linestyle)
